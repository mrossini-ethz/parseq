...
(define-simple-terminal (character-set) (rule item :quote t) (and (listp rule) (l= rule 2) (symbol= (first rule) 'char) (stringp (second rule)))
                        (if (char= (elt (second rule) 0) #\^)
                            (and (characterp item) (not (find item (expand-regexp-bracket-expression (subseq (second rule) 1)))))
